...
@riverpod
Future<UserModel> getVendorDetails(Ref ref, String vendorUID) async {
...
@riverpod
Future<void> updateWallet(Ref ref, OrderModel2 orderModel) async {
  try {
    // Read dependencies
    final getCoupon = ref.read(getPercentageOfCouponProvider(orderModel));
    final walletValue =
        await ref.read(getWalletProvider(orderModel.userID).future);

    // Ensure we handle business logic properly
    if (orderModel.paymentType == 'Wallet') {
      final total = orderModel.total + getCoupon;
      final newWalletValue = walletValue + total;
      ...
      Fluttertoast.showToast(
        msg: "Wallet has been updated successfully".tr(),
    ...
    // Log error and handle failures
    logger.e(
      "Failed to update wallet: $stack",
    );
    Fluttertoast.showToast(
      msg: "Failed to update wallet. Please try again.".tr(),
      toastLength: Toast.LENGTH_SHORT,
      gravity: ToastGravity.TOP,
      timeInSecForIosWeb: 1,
      fontSize: 14.0,
  ...
  if (orderStatus == 'Confirmed') {
    ...
    PushNotificationFunction().sendFirebaseNotification(
        NotificationsModel(
            uid: notificationID,
            heading: 'Order notification',
            content: 'Your order has was cancelled'),
        orderModel.userID,
        notificationID);
    ...
    if (orderStatus == 'Pending') {
      ...
      PushNotificationFunction().sendFirebaseNotification(
          NotificationsModel(
              uid: notificationID,
              heading: 'Order notification',
              content: 'Your order has been Pending'),
          orderModel.userID,
          notificationID);
    } else {
      // // loading.setLoading(true);
    ...
    if (element.vendorId.isNotEmpty) {
      FirebaseFirestore.instance
        ...
        if (orderModel.paymentType == 'Wallet') {
          ...
          PushNotificationFunction.sendPushNotification(
              'Admin Commission has been deducted',
              '${(-((element.quantity * element.selectedPrice * vendorCharge) / 100))}',
              value['tokenID']);
          historyVendor(
              HistoryModel(
                  message: 'Admin Commission has been deducted',
                  amount:
                      '${((element.quantity * element.selectedPrice) - ((element.quantity * element.selectedPrice * vendorCharge) / 100))}',
                  paymentSystem: 'Wallet',
                  timeCreated: DateTime.now()),
    ...
    if (orderModel.deliveryBoyID.isNotEmpty) {
      if (orderModel.paymentType == 'Wallet') {
        ...
        historyRider(
            HistoryModel(
                message: 'Admin Commission has been deducted',
                amount: '${(-((orderModel.deliveryFee! * riderCharge) / 100))}',
                paymentSystem: 'Wallet',
                timeCreated: DateTime.now()),
            orderModel.deliveryBoyID);
      }
      ...
      FirebaseFirestore.instance
        ...
        if (orderModel.paymentType == 'Wallet') {
          FirebaseFirestore.instance
            ...
            'isActive': false,
            'wallet': FieldValue.increment(((orderModel.deliveryFee!) -
                ((orderModel.deliveryFee! * orderModel.parcelAdminCommission!) /
                    100)))
          });
        } else {
          FirebaseFirestore.instance
            ...
            'isActive': false,
            'wallet': FieldValue.increment(-((orderModel.deliveryFee! *
                    orderModel.parcelAdminCommission!) /
                100))
          });
        }
        FirebaseFirestore.instance.collection('Admin').doc('Admin').update({
        ...
        if (orderModel.paymentType == 'Wallet') {
          PushNotificationFunction.sendPushNotification(
              'Payment for order',
              '${((orderModel.deliveryFee!) - ((orderModel.deliveryFee! * orderModel.parcelAdminCommission!) / 100))}',
              value['tokenID']);
        } else {
          PushNotificationFunction.sendPushNotification(
              'Admin commission has been deducted',
              '${-(((orderModel.deliveryFee! * orderModel.parcelAdminCommission!) / 100))}',
              value['tokenID']);
        }
      });
      historyRider(
...
@riverpod
Stream<List<String>> getRidersForAuto(Ref ref) {
  return FirebaseFirestore.instance
      .collection('riders')
      .where('isActive', isEqualTo: false)
      .where('approval', isEqualTo: true)
      .snapshots()
      .map((e) => e.docs.map((element) => element.id).toList());
}
