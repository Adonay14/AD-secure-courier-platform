import 'package:admin/models/payment_settings_model.dart';
...
class PaymentSettingsProvider extends _$PaymentSettingsProvider {
  ...
  updateEnebleYookassa() {
    state = state.copyWith(enableYookassa: !state.enableYookassa);
    return state.enableYookassa;
  }
  ...
  initialized() {
    ...
    FirebaseFirestore.instance
        ...
        .snapshots()
        .listen((value) {
      state = state.copyWith(enableYookassa: value['Yookassa']);
    });
    FirebaseFirestore.instance
        .collection('Payment System')
        .doc('Flutterwave')
    ...
    FirebaseFirestore.instance
      ...
      state = state.copyWith(
        ...
        getFlutterwaveEncriptedKey: value['Encryption Key'],
      );
    });
  }
  ...
  Future<void> updateFlutterwave(
      String publicKey, String secretKey, String encryptionKey) async {
    await paymentDetail.doc('Flutterwave').set({
      'Public key':
          publicKey.isEmpty ? state.getFlutterwavePublicKey : publicKey,
      'Secret Key':
          secretKey.isEmpty ? state.getFlutterwaveSecretKey : secretKey,
      'Encryption Key': encryptionKey.isEmpty
          ? state.getFlutterwaveEncriptedKey
          : encryptionKey
    });
  }
  ...
  Future<void> enableYookassa(bool enableYookassa) async {
    await paymentSystem
        .doc('Yookassa')
        .set({'Yookassa': enableYookassa});
  }
}
